...
function unboot_wym(elem){
  ...
  regex = new RegExp('<r:wym_radius_edit class="radius_tag radius_code radius_empty_tag">[\\s]*(\\S[^<]*\\S)[\\s]*</r:wym_radius_edit>', 'gim');
  m = content.match(regex);
  if (!(m == null)) {
    for (i=0; i<m.length; i++) {
      match = unescape(m[i].replace(regex, '<r:$1 />'));
      content = content.replace(m[i], match);
    }
  }

  // convert wym_radius_edit end tags back to radius tags
  regex = new RegExp('<r:wym_radius_edit class="radius_tag radius_code radius_end_tag">[\\s]*(\\S[^<]*\\S)[\\s]*</r:wym_radius_edit></r:wym_radius_edit>', 'gim');
  m = content.match(regex);
  if (!(m == null)) {
    for (i=0; i<m.length; i++) {
      match = unescape(m[i].replace(regex, '</r:$1>'));
  ...
  // convert wym_radius_edit end tags back to radius tags
  regex = new RegExp('<r:wym_radius_edit class="radius_tag radius_start_tag">[\\s]*<r:wym_radius_edit class="radius_tag radius_code">[\\s]*(\\S[^<]*\\S)[\\s]*</r:wym_radius_edit>', 'gim');
  m = content.match(regex);
  if (!(m == null)) {
    for (i=0; i<m.length; i++) {
      match = unescape(m[i].replace(regex, '<r:$1>'));
  ...
  regex = new RegExp('src="((http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?([\.\/]+)|([\.\/]+))page_attachments', 'g');
  m = content.match(regex);
  if(!(m == null)) {
    for(i=0; i<m.length; i++) {
      match = unescape(m[i].replace(regex, 'src="/page_attachments'));
      content = content.replace(m[i], match)
    }
  }

  // fix urls to assets
  regex = new RegExp('src="((http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?([\.\/]+)|([\.\/]+))assets', 'g');
  m = content.match(regex);
  if(!(m == null)) {
    for(i=0; i<m.length; i++) {
      match = unescape(m[i].replace(regex, 'src="/assets'));
      content = content.replace(m[i], match)
    }
  }
